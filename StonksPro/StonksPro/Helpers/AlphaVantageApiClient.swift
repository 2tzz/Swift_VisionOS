...
struct AlphaVantageNewsArticle: Codable, Identifiable {
    var id: String { url }
    let title: String
    let summary: String
    let source: String
    ...
    let topics: [AlphaVantageNewsTopic]?
...
struct AlphaVantageNewsTopic: Codable {
    let topic: String
    let relevance_score: String
}

struct AlphaVantageNewsResponse: Codable {
    let feed: [AlphaVantageNewsArticle]
}

class AlphaVantageApiClient {
    static func fetchNews(apiKey: String, topic: String = "", limit: Int = 50) async throws -> [AlphaVantageNewsArticle] {
        var urlString = "https://www.alphavantage.co/query?function=NEWS_SENTIMENT&apikey=\(apiKey)&limit=\(limit)"
        ...
        if let httpResponse = response as? HTTPURLResponse, !(200...299).contains(httpResponse.statusCode) {
            throw AlphaVantageError("HTTP error: \(httpResponse.statusCode)")
        }
        ...
        do {
            let decodedResponse = try JSONDecoder().decode(AlphaVantageNewsResponse.self, from: data)
            return decodedResponse.feed
        } catch {
            print("Decoding error: \(error)")
            throw AlphaVantageError("Failed to decode response: \(error.localizedDescription)")
        }
    }
